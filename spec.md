...
- **Menu Button On App Bar**: 菜单内容有“编辑”--用于编辑该middleware或page，不可编辑的时候为灰色不可用；还有"切换"--用于跳转到设备切换的界面。保留拓展能力用于未来开发。
...
- **Menu Button On App Bar**: 菜单内容有“编辑Page”--用于编辑该page，不可编辑的时候为灰色不可用；以及"编辑Middleware"，用于编辑Left Pane此时显示的middleware；还有"切换"--用于跳转到设备切换的界面。保留拓展能力用于未来开发。