...
# Stage 3 - Widget as a Page
## UI/UX *(mandatory)*
### Widget（更新）
#### WidgetPage概念
- 已有的**Widget**作为具体功能板块的概览或者快速操作面板，本身不具有完整功能。这次增加的**WidgetPage**作为Widget所代表的功能的完整面板呈现
- **WidgetPage**属于Widget的一种新的render展现方式，隶属于具体的**Widget**（是其中的一个render函数，只不过被抽象为WidgetPage这个**概念**），可以占据整个PageView的空间作为展现区域，以便展现完整功能。

#### WidgetPage展现区域
在RouterPageView中进行展示，也就是：
- 竖屏状态下，与一般的展示Stripe的Page一致
- 横屏状态下，与一般的展示Stripe的Page一致，在RightPane进行展示

#### 进入WidgetPage的方法
- 与已有的Page，Stripe，Widget三层展示系统分离，WidgetPage与普通的Page一样，可以通过Middleware直接进入。为了与普通的Page兼容，WidgetPage也有pageID，格式为`widget_{typeKey}`。
- 也可以通过长按Stripe中的Widget，直接跳转到对应的WidgetPage，实现快捷操作。

### Base Widget（更新）
- `BaseWidget`已经实现，需要在此基础上进行更新，以加入WidgetPage的实现支持
- 在`BaseWidget`类中留好`renderPage`的入口，然后由具体的Widget进行功能重写，与其他一般尺寸的render一致。

### Middleware（更新）
- `Middleware`已经实现，需要在此基础上进行更新以支持增加WidgetPage项
- 为了尽可能兼容原架构，每个Widget的WidgetPage也有pageID，格式为`widget_{typeKey}`
- 由于Widget没有写入数据库，因此在添加WidgetPage作为入口的时候需要额外查询`widgetCatalogProvider`查看已有widget

### Page（更新）
- `Page`已经实现，以展示Stripe和传统Widget样式，需要在此基础上更新以支持显示Widget的Page形式（WidgetPage）
- 为了最大程度减小更改，所有Widget都有pageID，格式为`widget_{typeKey}`，PageView识别到时Widget的时候，自动从对应的Widget的renderPage中拉取内容进行渲染。

## User Scenarios & Testing *(mandatory)*
### User Story 1 - 在Middleware中添加Widget
用户按照添加和编辑Middleware和Page相同的方法添加和编辑Widget

### User Story 2 - 从Middleware中跳转到Widget的Page形式
用户通过Middleware跳转到对应的Page，其中显示的是对应的Widget的Page内容

### User Story 3 - 从Widget跳转到Widget的Page形式
用户通过长按卡片形式的Widget，快速跳转到Page形式Widget。